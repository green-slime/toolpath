...
if __name__ == "__main__":
    ...
    note = f"3000渲染点训一次，观察此时500渲染点和501渲染点的渲染效果。"
    ...
    version_small = version_small + 1
    cfg.sample_size = 500
    cfg.max_epochs = 300
    small_foldername2 = cfg.output_foldername = f"output/large_{cfg.ocl_path_num}_{cfg.sample_size}_0.01_small_{version_small}/"
    update_output_config(cfg.sample_size, cfg.output_foldername)
    train.train(pretrained_pt=large_foldername+"/path_points_yz.pt")
    
    note2 = f"训练完成。相关输出文件夹为：\n" \
            f"large train: {large_foldername}\n" \
            f"small size render train (整除): {small_foldername}\n" \
            f"small size render train2 (非整除): {small_foldername2}\n"
