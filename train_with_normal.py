"""
先以少于变量数的采样数进行ot，然后再进行渲染loss
"""

from batch_intersect_min import batch_intersect, batch_intersect_with_cuda, batch_intersect_for_render, batch_intersect_for_height, batch_intersect_for_normals
import bvh
import render
import torch
import utils
...
def normal_loss_with_cuda(Ps, x_grid, path_points_yzs, target_normals, device='cpu'):
    """
    计算法向量损失，使用 CUDA 扩展。
    """
    _, _, loss, grad = batch_intersect_for_normals(Ps, cfg.R, cfg.sample_size, x_grid, path_points_yzs, batch_size=cfg.batch_size, device=device, alpha=cfg.alpha, MAX_HEIGHT=cfg.zmax, target_normals=target_normals)
    return loss, grad
...
def train():
    ...
    #path_points_yz = use_yz_pt(f"{cfg.output_foldername}/path_points_yz.pt", device=device)  # 尝试加载预训练的路径点
    
    #fixed_receiver_points = get_fixed_receiver_points(device=device)  # 获取固定的接收器点
    with torch.no_grad():
        fixed_receiver_points, fixed_heights, fixed_normals = get_target_pos.get_target_pos2(device=device)  # 获取目标接收器点
        ...
        #make_obj(cfg.sample_size_for_obj, path_points_yz, x_grid, device=device, output_file=f'{cfg.output_foldername}/init.obj')  # 保存 OBJ 文件
    ...
    optimizer = torch.optim.Adam([path_points_yz], cfg.normal_learning_rate, betas=(0.9, 0.99), eps=1e-8, amsgrad=False)
    ...
    change_at_epoch = cfg.max_epochs+1
    ...
    for epoch in range(cfg.max_epochs):
        ...
        if epoch < change_at_epoch:
            normal_loss, grad = normal_loss_with_cuda(Ps, x_grid, path_points_yz, fixed_normals, device=device)  # 计算法向量损失
        ...
        if epoch < change_at_epoch:
            loss = normal_loss + overlayloss  # 使用 OT 损失和覆盖损失的加权和
        else:
        ...
        if epoch % cfg.print_epochs == 0 or epoch == cfg.max_epochs - 1:
            ...
            if epoch < change_at_epoch:
                print(f"Epoch {epoch + 1}/{cfg.max_epochs}, Loss: {loss.item():.6f}, normal Loss: {normal_loss.item():.6f}, Overlay Loss: {overlayloss.item():.6f}")
            else:
        ...
        if (epoch >= 0 and epoch % cfg.draw_epochs == 0) or epoch == cfg.max_epochs - 1:
            with torch.no_grad():
                torch.save(path_points_yz, f"{cfg.output_foldername}/path_points_yz_normal_{epoch}.pt")  # 保存路径点
                #receiver_points = get_receiver_points(Ps, path_points_yz, x_grid, device=device)  # 获取当前接收器点
                render_result = prepare_for_render.get_render_result(receiver_points, info_list)
                #surface_points, dirs = get_dirs(Ps, path_points_yz, x_grid, device=device)  # 获取当前方向向量
