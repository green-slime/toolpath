...
from batch_intersect_min import intersect_for_height
def height_loss_new(Ps, path_points_yzs, target_height, device='cpu'):
    """
    计算高度损失，使用新的 CUDA 扩展。
    """
    _, _, loss, grad = intersect_for_height(Ps, cfg.R, cfg.sample_size, path_points_yzs, MAX_HEIGHT=cfg.zmax, target_height=target_height)
    return loss, grad
...
def train(pretrained_pt=None):
    ...
    optimizer = torch.optim.Adam([path_points_yz], cfg.learning_rate*0.1, betas=(0.9, 0.99), eps=1e-8, amsgrad=True)
    ...
    for epoch in range(cfg.max_epochs):
        ...
        if epoch > change_at_epoch:
            ...
            height_loss, grad = height_loss_new(Ps, path_points_yz, fixed_heights, device=device)
        else:
        ...
        if epoch > change_at_epoch:
            loss = height_loss + overlayloss   # 使用 OT 损失和覆盖损失的加权和
        else:
        ...
        if epoch % cfg.print_epochs == 0 or epoch == cfg.max_epochs - 1:
            ...
            if epoch > change_at_epoch:
                print(f"Epoch {epoch + 1}/{cfg.max_epochs}, Loss: {loss.item():.6f}, height Loss: {height_loss.item():.6f}, Overlay Loss: {overlayloss.item():.6f}")
            else:
...
if __name__ == "__main__":
    cfg.max_epochs = 2000
    train(pretrained_pt=f"output/large_{cfg.ocl_path_num}_800_0.01_ot_2/path_points_yz_ot.pt")
