...
__global__ void new_compute_diff(ScalarType* d_compute_tri_area, ScalarType* d_compute_tri_area_grad, int* d_faces, ScalarType* d_proj_xy,
     ScalarType* d_proj_xy_render_grad, ScalarType* d_total_gray_value, ScalarType* d_render_diff, int* d_face_num,  ScalarType* d_edge_len, int* d_img_size)
{
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if(tid < d_face_num[0])
    {
        ...
        int point_idx[3] = { d_faces[3*tid], d_faces[3*tid+1], d_faces[3*tid+2]};
        ScalarType tri_coor[6];
        ScalarType big_area_grad[6] = {0};
        ScalarType small_area_grad[6];
        ScalarType clip_grad[36] = {0};
        ScalarType total_grad[6] = {0};

        int poly_num = 3;
        int img_size = d_img_size[0];
        ScalarType max_edge_len=d_edge_len[0];
        ScalarType expand_num = img_size/max_edge_len;
        for(int i = 0; i < 3; i++)
        {
            tri_coor[2*i] = d_proj_xy[2*point_idx[i]]*expand_num;
            tri_coor[2*i+1] = d_proj_xy[2*point_idx[i]+1]*expand_num;
        }
        for(int i = 0; i < 6; i++)
        {
            clip_grad[7*i] = 1;
        ...
        ScalarType big_area = compute_grad_area( tri_coor, big_area_grad, poly_num);
        ...
        ScalarType clipsquare[4];
        ScalarType new_poly[max_edge_num];
        ScalarType clip_poly_grad[3*max_edge_num];
        ScalarType new_poly_grad[max_edge_num] = {0};
        int new_poly_num;
        ScalarType local_area_diff = 0;
        ...
        for(int i_x = low_bound_x; i_x < high_bound_x; i_x++)
        {
            for(int i_y = low_bound_y; i_y < high_bound_y; i_y++)
            {
                ...
                grad_clip_Polygon( tri_coor, clip_grad, poly_num, clipsquare, new_poly, clip_poly_grad, new_poly_num);

                for(int i = 0; i < new_poly_num*2; i++)
                {
                    new_poly_grad[i] = 0;
                }

                ScalarType area = compute_grad_area( new_poly, new_poly_grad, new_poly_num);
                accumulate_grad( small_area_grad, clip_poly_grad, new_poly_grad, new_poly_num);

                if(big_area!=0)
                {
                    ScalarType render_diff = d_render_diff[grid];
                    local_area_diff = local_area_diff + 2 * render_diff * area/big_area;
                    for(int i = 0; i < 6; i++)
                    {
                        total_grad[i] = 2 * render_diff * gray_value
                            * expand_num * (small_area_grad[i]*big_area - big_area_grad[i]*area)/(big_area * big_area);
                    }

                    atomicAdd(&d_proj_xy_render_grad[2*point_idx[0]],total_grad[0]);
                    atomicAdd(&d_proj_xy_render_grad[2*point_idx[0]+1],total_grad[1]);
                    atomicAdd(&d_proj_xy_render_grad[2*point_idx[1]],total_grad[2]);
                    atomicAdd(&d_proj_xy_render_grad[2*point_idx[1]+1],total_grad[3]);
                    atomicAdd(&d_proj_xy_render_grad[2*point_idx[2]],total_grad[4]);
                    atomicAdd(&d_proj_xy_render_grad[2*point_idx[2]+1],total_grad[5]);
                }   
            }
        }
        local_area_diff = local_area_diff * d_total_gray_value[0];
       atomicAdd(&d_compute_tri_area_grad[tid] , local_area_diff);
    }
}
...
std::vector<torch::Tensor> new_diff(torch::Tensor compute_tri_area, torch::Tensor faces, torch::Tensor proj_xy,
    torch::Tensor total_gray_value, torch::Tensor real_picture, torch::Tensor edge_len, int img_size)
{
    cudaSetDevice(compute_tri_area.get_device());
    int threadnum = 256;
    ...
    ScalarType* d_proj_xy_render_grad;
    cudaMalloc(&d_proj_xy_render_grad, 2 * points_num * sizeof(ScalarType));
    cudaMemset(d_proj_xy_render_grad, 0, 2 * points_num * sizeof(ScalarType));
    
    int faces_num = compute_tri_area.numel();
    ScalarType* d_render_tri_area_grad;
    cudaMalloc(&d_render_tri_area_grad, faces_num * sizeof(ScalarType));
    cudaMemset(d_render_tri_area_grad, 0, faces_num * sizeof(ScalarType));

    int* d_face_num;
    cudaMalloc(&d_face_num, sizeof(int));
    cudaMemcpy(d_face_num, &faces_num, sizeof(int), cudaMemcpyHostToDevice);

    compute_render<<<(faces_num + threadnum - 1) / threadnum, threadnum>>>(
        d_compute_tri_area, d_faces, d_proj_xy, 
        d_total_gray_value, d_render_result, d_face_num, d_edge_len, d_img_size);

    compute_sub<<<(img_size*img_size+ threadnum - 1)/threadnum,threadnum>>>( d_render_result, d_real_picture, d_render_diff, d_img_size);

    new_compute_diff<<<(faces_num + threadnum - 1) / threadnum, threadnum>>>( d_compute_tri_area, d_render_tri_area_grad, d_faces, d_proj_xy, 
        d_proj_xy_render_grad, d_total_gray_value, d_render_diff, d_face_num, d_edge_len, d_img_size);
    ...
    torch::Tensor proj_xy_grad = torch::from_blob(d_proj_xy_render_grad, {points_num , 2}, torch::kFloat).to(torch::kCUDA);
    proj_xy_grad = proj_xy_grad.clone();
    torch::Tensor tri_area_grad = torch::from_blob(d_render_tri_area_grad, {faces_num}, torch::kFloat).to(torch::kCUDA);
    tri_area_grad = tri_area_grad.clone();

   cudaFree(d_proj_xy_render_grad);
   cudaFree(d_render_tri_area_grad);
   cudaFree(d_face_num);
   cudaFree(d_render_result);
   cudaFree(d_render_diff);

   return {render_result, render_diff, proj_xy_grad, tri_area_grad};
}
...
std::vector<torch::Tensor> diff( torch::Tensor compute_tri_area, torch::Tensor faces, torch::Tensor proj_xy, torch::Tensor total_gray_value, torch::Tensor real_picture, torch::Tensor real_sobel_x, torch::Tensor real_sobel_y, torch::Tensor edge_len, int img_size)
{
    ...
    points_diff_l1 = points_diff_l1.clone();
    ...
    torch::Tensor sobel_x_diff = torch::from_blob(d_sobel_x_diff, {img_size, img_size}, torch::kFloat).to(torch::kCUDA);
    sobel_x_diff = sobel_x_diff.clone();
    torch::Tensor sobel_y_diff = torch::from_blob(d_sobel_y_diff, {img_size, img_size}, torch::kFloat).to(torch::kCUDA);
    sobel_y_diff = sobel_y_diff.clone();
    ...
    return {render_diff,points_diff,tri_area_diff,render_result,points_sobel_diff, sobel_x_diff, sobel_y_diff, points_diff_l1};
}
